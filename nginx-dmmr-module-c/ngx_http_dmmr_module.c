...
ngx_http_dmmr_handler(ngx_http_request_t *r)
{
    ...
    if (rc != NGX_OK) {
        if (rc == NGX_HTTP_NOT_FOUND) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                          "dmmr: no route matched for URI '%V'", &r->uri);
    ...
    if (rc != NGX_OK) {
        if (rc == NGX_HTTP_UNAUTHORIZED || rc == NGX_HTTP_FORBIDDEN) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
...
ngx_http_dmmr_service(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (kcf->services == NULL) {
        kcf->services = ngx_array_create(cf->pool, 4,
                                         sizeof(ngx_http_dmmr_service_t));
        if (kcf->services == NULL) {
...
/* Criação de contexto de configuração */
static void *
